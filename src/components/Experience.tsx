...
const EXPERIENCES: Experience[] = [
  {
    ...
    type: "Part-time",
    description: [
      "Building the core backend of Orga Africa's food-ordering platform in NestJS (modular monolith) — 7 isolated domain modules (orders, restaurants, menus, payments, notifications, delivery, auth) behind clean interfaces, sharing infrastructure while enabling independent iteration per domain.",
      "Engineered the full order lifecycle: cart validation, real-time menu availability, atomic stock reservation, kitchen routing, 5-state status machine (placed → preparing → ready → dispatched → delivered) and refund flows — using idempotency keys and optimistic locking to eliminate double-submissions under concurrent peak load.",
      "Designed secure REST APIs with NestJS Guards, Pipes and JWT + refresh-token auth, enforcing RBAC across 3 roles (customer, restaurant operator, admin) — reducing unauthorised access incidents to zero since launch.",
      "Modelled PostgreSQL schemas with TypeORM and tuned query performance: composite indexes on high-frequency filters and batch writes sustaining <200ms p95 response times under peak ordering load.",
      "Deployed Redis for distributed session management, sliding-window rate limiting (per user/IP), distributed locking against duplicate submissions, and menu/listing read caching — cutting direct DB load by ~40% on hot paths.",
      "Offloaded async workloads (settlement reports, push notifications, analytics roll-ups, invoice generation) to AWS Lambda — decoupling them from the request path and reducing compute costs vs always-on alternatives.",
      "Containerised the stack with Docker across dev/staging/prod (AWS RDS, ElastiCache, Lambda, S3, CloudWatch) with structured logging and alerting for real-time visibility into order throughput, error rates and latency percentiles.",
      "Maintaining >80% test coverage with Jest: unit tests for pricing engine and order state machine, integration tests for repository layers, and contract tests preventing regressions before each deploy.",
    ],
